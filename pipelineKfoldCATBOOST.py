...
def Crear_modelo_CATBOOST(X_train, y_train):
    ...
    param_search = {
        'iterations': Integer(50, 150),  # Menos iteraciones = más rápido
        'depth': Integer(4, 6),          # Árboles más pequeños
        'learning_rate': Real(0.05, 0.2, prior='log-uniform'),
        'l2_leaf_reg': Integer(1, 3),
        'grow_policy': Categorical(['SymmetricTree']),  # Solo 1 opción
    }
...
def normalizacion_CATBOOST(X_train, X_test):
...
# Función de entrenamiento base para CATBOOST
def entrenamiento_base_CATBOOST(X_train, X_test, y_train, y_test):
    X_train_scaled, X_test_scaled = normalizacion_CATBOOST(X_train, X_test)
    
    model = Crear_modelo_CATBOOST(X_train_scaled, y_train)
...
# Función para aplicar SMOTE con CATBOOST
def Aplicar_Smote_CATBOOST(X_train, X_test, y_train, y_test):
    smote = SMOTE(random_state=42)
    X_resampled_smote, y_resampled_smote = smote.fit_resample(X_train, y_train)
    X_train_scaled, X_test_scaled = normalizacion_CATBOOST(X_resampled_smote, X_test)

    model = Crear_modelo_CATBOOST(X_train_scaled, y_resampled_smote)
    y_pred = model.predict(X_test_scaled)
    a,b = confusion_matrix(y_test, y_pred)
    exactitud,sensibilidad,especificidad,vpp,vpn,PRC,AUC,tp,tn,fp,fn = calcular_metricas(a,b,y_test,y_pred,model,X_test_scaled)
...
def Aplicar_VAE_CATBOOST(X_train, X_test, y_train, y_test):
    ...
    X_train_reshaped,X_test_reshaped = normalizacion_CATBOOST(X_train_balanced, X_test)
    
    model = Crear_modelo_CATBOOST(X_train_reshaped,y_train_balanced)
...
def Aplicar_DifussionModel_CATBOOST(X_train, X_test, y_train, y_test):
    ...
    #    X_train_reshaped,X_test_reshaped,X_test_total_scaled = normalizacion_CATBOOST(X_train_balanced, X_test,X_test_total)

    X_train_reshaped,X_test_reshaped = normalizacion_CATBOOST(X_train_resampled, X_test)
    
    model = Crear_modelo_CATBOOST(X_train_reshaped,y_train_resampled)
...
def Aplicar_Copulas_CATBOOST(X_train, X_test, y_train, y_test):
    df_xtrain = pd.DataFrame(X_train)
    df_xtrain.columns = df_xtrain.columns.astype(str)  # Convertir nombres a strings
...
# Función para evaluar métricas con CATBOOST                 inscripcion_resultados_CATBOOST(best_accuracy,best_precision,best_recall,best_f1,best_PRC,best_AUC)

def inscripcion_resultados_CATBOOST(archivo, bacteria,tipo_entrenamiento,exactitud,sensibilidad,especificidad,vpp,vpn,PRC,AUC,tiempo,tp,tn,fp,fn):
    ...
    for bacteria in columnas_bacterias  : #   columnas_bacterias  ['CEFTRIAXONA'] 
        try:
            ...
            for fold, (train_index, val_index) in enumerate(skf.split(X, y)):
                ...
                tipo_entrenamiento,exactitud,sensibilidad,especificidad,vpp,vpn,PRC,AUC,tp,tn,fp,fn = entrenamiento_base_CATBOOST(X_train_fold, X_val_fold, y_train_fold, y_val_fold)
                end_time = time.time()
                tiempo = round(end_time - start_time,2)


                inscripcion_resultados_CATBOOST(archivo, bacteria,tipo_entrenamiento,exactitud,sensibilidad,especificidad,vpp,vpn,PRC,AUC,tiempo,tp,tn,fp,fn)
            ...
            for fold, (train_index, val_index) in enumerate(skf.split(X, y)):
                ...
                tipo_entrenamiento,exactitud,sensibilidad,especificidad,vpp,vpn,PRC,AUC,tp,tn,fp,fn = Aplicar_Smote_CATBOOST(X_train_fold, X_val_fold, y_train_fold, y_val_fold)
                end_time = time.time()
                tiempo = round(end_time - start_time,2)
            ...
            for fold, (train_index, val_index) in enumerate(skf.split(X, y)):
                ...
                tipo_entrenamiento,exactitud,sensibilidad,especificidad,vpp,vpn,PRC,AUC,tp,tn,fp,fn = Aplicar_VAE_CATBOOST(X_train_fold, X_val_fold, y_train_fold, y_val_fold)
                end_time = time.time()
                tiempo = round(end_time - start_time,2)
            ...
            for fold, (train_index, val_index) in enumerate(skf.split(X, y)):
                ...
                tipo_entrenamiento,exactitud,sensibilidad,especificidad,vpp,vpn,PRC,AUC,tp,tn,fp,fn = Aplicar_DifussionModel_CATBOOST(X_train_fold, X_val_fold, y_train_fold, y_val_fold)
                end_time = time.time()
                tiempo = round(end_time - start_time,2)
            ...
            for fold, (train_index, val_index) in enumerate(skf.split(X, y)):
                ...
                tipo_entrenamiento,exactitud,sensibilidad,especificidad,vpp,vpn,PRC,AUC,tp,tn,fp,fn = Aplicar_Copulas_CATBOOST(X_train_fold, X_val_fold, y_train_fold, y_val_fold)
                end_time = time.time()
                tiempo = round(end_time - start_time,2)
            ...
            #Entrenamiento VAE
            tipo_entrenamiento,exactitud,sensibilidad,especificidad,vpp,vpn,PRC,AUC,tp,tn,fp,fn = Aplicar_VAE_CATBOOST(X_train, X_test, y_train, y_test)
            inscripcion_resultados_CATBOOST(archivo, bacteria,tipo_entrenamiento,exactitud,sensibilidad,especificidad,vpp,vpn,PRC,AUC,tp,tn,fp,fn)
            del tipo_entrenamiento,exactitud,sensibilidad,especificidad,vpp,vpn,PRC,AUC,tp,tn,fp,fn
            gc.collect()
             
            #Entrenamiento Diffusion model
            tipo_entrenamiento,exactitud,sensibilidad,especificidad,vpp,vpn,PRC,AUC,tp,tn,fp,fn = Aplicar_DifussionModel_CATBOOST(X_train, X_test, y_train, y_test)
            inscripcion_resultados_CATBOOST(archivo, bacteria,tipo_entrenamiento,exactitud,sensibilidad,especificidad,vpp,vpn,PRC,AUC,tp,tn,fp,fn)
            del tipo_entrenamiento,exactitud,sensibilidad,especificidad,vpp,vpn,PRC,AUC,tp,tn,fp,fn
            gc.collect()
            ...
            df_resultados_aux.to_excel('resultadosExcelCATBOOST/CATBOOST-'+archivo+'.xlsx',index=False)
            Correo('se inscribio',archivo)
Correo('Finalizo todo el programa','CATBOOST')
